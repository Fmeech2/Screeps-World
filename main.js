...
телаКриповПоИмени = {
    ...
    апдейтер2: {
        роль: "⚡",
        тело: [WORK, CARRY, MOVE, MOVE, MOVE]
    },
    курьер_энергии2_простой: {//курьер_энергии2_простой
        роль: "📦",
        тело: [CARRY, MOVE, CARRY, MOVE, CARRY, MOVE]
    },
    строитель2: {
        роль: "🏗️",
        тело: [WORK, CARRY, CARRY, MOVE, MOVE]
    },
}
module.exports.loop = function () {
    ...
    function фаза2() {
        let имяКрипа = "Спейс марин фазы 2";
        let спавн1 = "Spawn1";
        let уровеньКонтроллераВКомнатеСпавна = Game.spawns[спавн1].room.controller.level;
        if (крипыФазы2.length >= Memory.фаза.последовательностьКрипов) {
            let началСпавнКрипа = СпавнКрипа(
                спавн1
                , имяКрипа
                , телаКриповПоИмени[крипыФазы2[Memory.фаза.последовательностьКрипов - 1]].роль
                , телаКриповПоИмени[крипыФазы2[Memory.фаза.последовательностьКрипов - 1]].тело
            );
            if (началСпавнКрипа) { //двигаемся к спавну следующего крипа, после успешного спавна текущего
                Memory.фаза.последовательностьКрипов = Memory.фаза.последовательностьКрипов + 1;
            }
        ...
        for (var имя in Game.creeps) {
            var крип = Game.creeps[имя];
            //🚜 фермер
            if (крип.memory.роль == телаКриповПоИмени.фермер2.роль) {
                фермер2(крип);
            }
            //⚡апдейтер
            else if (крип.memory.роль == телаКриповПоИмени.апдейтер2.роль) {
                апдейтер1(крип);
            }
            //🏗️ строитель
            else if (крип.memory.роль == "🏗️") {
            ...
            //📦 курьер
            else if (крип.memory.роль == телаКриповПоИмени.курьер_энергии2_простой.роль) {
                курьер_энергии2_простой(крип);
            }
        }
    }
    ...
    /** @param {Creep} крип **/
    function планировщик1(крип) {

    }
    function фермер2(крип) {
        var НномерЗолотойШахты = крип.memory.номер % 2;
        var золотодобытчик1 = крип.room.find(FIND_SOURCES)[НномерЗолотойШахты];
        if (крип.store[RESOURCE_ENERGY] == 0 && крип.memory.статус != 'иду ⛏️') {
            крип.memory.статус = 'иду ⛏️';
        }
        ...
        if (крип.memory.статус == 'Добываю 💎️') {
            if (крип.store.getFreeCapacity() == 0) {
                ...
                крип.moveTo(крип.room.find(FIND_SOURCES)[НномерЗолотойШахты],
                    {
                        visualizePathStyle: {
                            stroke: 'black',   // Цвет линии (HEX)
                            lineStyle: 'solid', // Тип линии: пунктир"dashed", микропунктир"dotted" или линия"solid"
                ...
                крип.harvest(крип.room.find(FIND_SOURCES)[НномерЗолотойШахты]);
            }
        }
        if (крип.memory.статус == 'Разгружаю 💰️') {
            let курьер = найтиКурьера(крип);
            if (курьер) {
                if (крип.pos.getRangeTo(курьер) < 3) {
                    крип.transfer(курьер, RESOURCE_ENERGY);
                    крип.moveTo(курьер);
                }
            }
        }
    ...
    /** @param {Creep} крип **/
    function курьер_энергии2_простой(крип) {
        var НномерЗолотойШахты = крип.memory.номер % 2;
        var золотодобытчик1 = крип.room.find(FIND_SOURCES)[НномерЗолотойШахты];
        if (крип.store[RESOURCE_ENERGY] == 0 && крип.memory.статус != 'иду забирать 📦') {
            крип.memory.статус = 'иду забирать 📦';
        }
        if (крип.memory.статус == 'иду забирать 📦') {
            if (крип.pos.getRangeTo(золотодобытчик1) < 2 && крип.store.getFreeCapacity() != 0) {
                крип.memory.статус = 'В процессе получения 📦';
            }
            else if (крип.store.getFreeCapacity() != 0) {
                крип.moveTo(найтиОжидающегоФермера2(крип),
                    {
                        visualizePathStyle: {
                            stroke: '#ffd700'
                        }
                ...
                крип.memory.статус = 'Доставляю 📦'
            }
        }
        if (крип.memory.статус == 'В процессе получения 📦') {
            if (крип.store.getFreeCapacity() == 0) {
                крип.memory.статус = 'Доставляю 📦';
            }
            else {
                крип.say(анимация("часики"));
            }
        }
        if (крип.memory.статус == 'Доставляю 📦') {
            var хранилищаСпавна = кудаВыгружатьЭнергиюСпавна(крип);
            крип.moveTo(хранилищаСпавна,
                {
                    visualizePathStyle: {
                        stroke: '#00FFFF',   // Цвет линии (HEX)
                        lineStyle: 'solid', // Тип линии: пунктир"dashed", микропунктир"dotted" или линия"solid"
            ...
            крип.transfer(хранилищаСпавна, RESOURCE_ENERGY);
        }
    }
    /** @param {Creep} крип **/
    ...
    function найтиКурьера(крип) {
        let курьер = false;
        let ближайшееРасстояние = 100;
        for (var имя in Game.creeps) {
            let курочка = Game.creeps[имя];
            if (курочка.memory.роль == телаКриповПоИмени.курьер_энергии2_простой.роль) {
                let расстояниеПоХ = Math.abs(крип.pos.x - курочка.pos.x);
                let расстояниеПоУ = Math.abs(крип.pos.y - курочка.pos.y);
                расстояние = расстояниеПоХ + расстояниеПоУ;
                if (ближайшееРасстояние > расстояние)
    ...
    function найтиОжидающегоФермера2(крип) {
        let работяга = false;
        let ближайшееРасстояние = 100;
        for (var имя in Game.creeps) {
            let фермер = Game.creeps[имя];
            if (фермер.memory.роль == телаКриповПоИмени.фермер2.роль && фермер.store.getFreeCapacity()==0) {
                let расстояниеПоХ = Math.abs(крип.pos.x - фермер.pos.x);
                let расстояниеПоУ = Math.abs(крип.pos.y - фермер.pos.y);
                расстояние = расстояниеПоХ + расстояниеПоУ;
                if (ближайшееРасстояние > расстояние)
                    ближайшееРасстояние = расстояние;
                работяга = фермер;
